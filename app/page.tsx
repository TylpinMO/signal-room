        ...
        <div className="dashboard">
          <section id="overview" className="page-heading">
            ...
            <div className="heading-controls">
              ...
              <div className="range-tabs" role="group" aria-label="Date range">{(["7d", "30d", "90d"] as Range[]).map((item) => <button className={range === item ? "active" : ""} onClick={() => setRange(item)} key={item}>{item}</button>)}</div>
            </div>
          </section>
          ...
          <section className="chart-card" aria-labelledby="growth-title">
            ...
            <div className="chart-area">
              ...
              <div className="bars" role="img" aria-label={`Revenue bars for ${range}`}>
          ...
          <div className="analysis-grid">
            <section id="acquisition" className="table-card">
              ...
              <div className="filter-row" role="group" aria-label="Channel filter">{(["All", "Organic", "Paid", "Partners"] as Channel[]).map((item) => <button className={channel === item ? "active" : ""} onClick={() => setChannel(item)} key={item}>{item}</button>)}</div>
              <div className="channel-table" role="region" tabIndex={0} aria-label="Acquisition channels table">
          ...
          <section id="retention" className="cohort-card">
            ...
            <div className="cohort-grid" role="region" tabIndex={0} aria-label="Weekly retention cohort table">
              ...
              {[0.94,0.82,0.73,0.65, 0.91,0.79,0.69,0.58, 0.96,0.84,0.76,0.67, 0.93,0.81,0.72,0.62].map((intensity, index) => <i key={index} style={{ backgroundColor: `hsl(72 78% ${Math.round(88 - intensity * 22)}%)` }}><span>{Math.round(intensity * 52)}%</span></i>)}
            </div>
          </section>
        </div>
      </main>
